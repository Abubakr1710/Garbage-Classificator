...
if choice == 'Home':
    ...
    while run:
        ...
        closing=cv2.morphologyEx(gframe, cv2.MORPH_CLOSE, kernel, iterations=2)
        retval, threshold= cv2.threshold(closing, 100,255, cv2.THRESH_BINARY_INV)
        contours, hierarchy = cv2.findContours(threshold, cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_NONE)
        sorted_contours=sorted(contours, key=cv2.contourArea, reverse=True)
        ncframe = frame.copy()
        x,y,w,h = cv2.boundingRect(sorted_contours[0])
        rect=cv2.rectangle(ncframe, (x,y), (x+w, y+h),(0,255,0), 2)
        dr=ncframe[y:y+h, x:x+w]
        FRAME_WINDOW.image(ncframe)
    else:
        st.write('Stopped')
